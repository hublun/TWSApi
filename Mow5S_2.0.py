...
#==================================================Reat time bar =============================================================
   ...
   #========================================
    ...
    def realtimeBar(self, reqId: TickerId, time:int, open_: float, high: float, low: float, close: float,
        ...
        if (beta := np.sign(hi_beta)*np.sign(lo_beta)) == -1.0:
            print("No beta {0:2.2} need to evaluate t-value{1:2.2f}".format(beta, tv))
            return

        #====== slope control to make sure new low or new high ================    

        print("\t\t>>>>>>>>>>>>>>[t-Value: {0:+3.2f}] [Price: {1:>4.2f}] [Bar: {2:2.2f}] [Sign: {3:2.1f} {4:2.1f}]".format(tv, close, currentBar, np.sign(hi_beta), np.sign(lo_beta)))
        #======================== order preparation ==========
        ...
        cond_too_many_longs =  tv < -0.0 and self.pos >= (self.Max_lp-old_opens[0])
        cond_too_many_shorts = tv > 0.0 and self.pos <=  (self.Max_sp + old_opens[1])
