...
#---------------------------------------------------------
        ...
        prb = self.tick*(low//self.tick) - 4*self.tick       
        prs = self.tick*(high//self.tick) + 4*self.tick
        ...
        if high < self.avg_price and (res:=self.pos + old_opens[0]) < 0:
            p = abs(res)
            while (p >0):
                lb_odr =  OrderSamples.LimitOrder("BUY", 1, prb+(abs(self.pos)-p)*2*self.tick)  
        ...
        if low > self.avg_price and (res:=self.pos - old_opens[1]) > 0:
            p = res
            while (p >0):
                ls_odr =  OrderSamples.LimitOrder("SELL", 1, prs-(abs(self.pos)-p)*2*self.tick)  
