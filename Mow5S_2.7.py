...
#==================================================Reat time bar =============================================================
   ...
   #========================================
    ...
    def realtimeBar(self, reqId: TickerId, time:int, open_: float, high: float, low: float, close: float,
        ...
        print(datetime.datetime.fromtimestamp(time, tz=None),
            "\t[ {0:+2.0f} ${1:<4.2f}]".format(self.pos, self.avg_price),          
            "\t++ {:4.2f} --".format(close),
            "\t[OB: {0:2.0f} OS: {1:2.0f}]".format(self.open_orders[0], self.open_orders[1]),
            "\t[LB$ {0:2.2f} LS$ {1:2.2f}]".format(self.last_prices[0], self.last_prices[1]),
            self.ctr_bs)
        ...
        self.reqPositions()
        old_opens = self.open_orders # store the current open orders for decision making in this cycle
        self.open_orders =[0,0] # reset the open orders for the next cycle
        self.reqOpenOrders()  
        ...
        prb = math.floor(low-self.delta*self.open_orders[0])
        lb_odr =  OrderSamples.LimitOrder("BUY", 1, prb)  
        prs = math.ceil(high+self.delta*self.open_orders[1])
        ls_odr = OrderSamples.LimitOrder("SELL", 1, prs)

        if self.pos < 0 and self.ctr_bs[0] == 0 and high < self.avg_price:
            self.placeOrder(self.nextOrderId(), self.contrakt, lb_odr)
            self.ctr_bs[0] +=1 
            return
 
        if self.pos > 0 and self.ctr_bs[1] == 0 and low > self.avg_price:
            self.placeOrder(self.nextOrderId(), self.contrakt, ls_odr)
            self.ctr_bs[1] += 1
            return
        #================================================================================
        ...
        if (abs(tv) < 2.98):
            return
        #========================== slope calculation =======================
        hi_beta =np.sign((high - self.hhillow[1])/(time - self.hhillow[0]+5))
        lo_beta =np.sign((low - self.hhillow[3])/(time - self.hhillow[2]+5))

        print(self.hhillow, hi_beta, lo_beta)
        if (beta := hi_beta*lo_beta) == -1.0:
            print("with beta {0:2.2} =-1 NO need to evaluate t-value {1:2.2f}".format(beta, tv))
            return

        #====== slope control to make sure new low or new high ================    

        print(">>>[t-Value: {0:+3.2f}] [Price: {1:>4.2f}] [Bar: {2:2.2f}] [Sign: {3:2.6f} {4:2.6f}]".format(tv, close, currentBar, hi_beta, lo_beta))
        #======================== order preparation ==========
        ...
        fut_order = OrderSamples.LimitOrder("HOLD", 1, pr)

        if tv < 0 and lo_beta<=0:
            fut_order.action = "BUY"
        elif tv>0 and hi_beta>=0:
            fut_order.action = "SELL"
        else:
            print("No Deal")
            return
        ...
        #====================Not releasing too many orders==========================================
        if fut_order.action == "SELL" and old_opens[1] > 0:
            print("Overselling Bye !")
            return
        if fut_order.action == "BUY" and old_opens[0] > 0:
            print("Overbuying Bye !")
            return        
        
        # ----------------------- too many short and long pos --------------------------
        cond_too_many_longs =  fut_order.action == "BUY" and self.pos >= self.Max_lp
        cond_too_many_shorts = fut_order.action == "SELL" and self.pos <=  self.Max_sp
        ...
        if fut_order.action == "HOLD":
            return
        elif fut_order.action == "BUY":
            self.ctr_bs[0] += 1
            self.placeOrder(self.nextOrderId(), self.contrakt, fut_order)
        elif fut_order.action == "SELL":
            self.ctr_bs[1] += 1
            self.placeOrder(self.nextOrderId(), self.contrakt, fut_order)
        else:
            return
        print(self.ctr_bs)
        # == updating open orders outstanding ====
