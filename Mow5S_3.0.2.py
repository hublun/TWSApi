...
#==================================================Reat time bar =============================================================
   ...
   #========================================
    ...
    def realtimeBar(self, reqId: TickerId, time:int, open_: float, high: float, low: float, close: float,
        ...
        #========= requesting open orders every cycle ================
        ...
        gamma = currentBar/(self.hhillow[1]-self.hhillow[3]+0.01)
        self.cul_ratio += gamma
        print(datetime.datetime.fromtimestamp(time, tz=None),
            "\t[{0:+2.0f} ${1:<4.2f}]".format(self.pos, self.avg_price),
            "\t {:0> 2.0%}".format(self.cul_ratio),          
            "\t~~{:4.2f}~~".format(close),
            "[OB: {0:1.0f} OS: {1:1.0f}]".format(old_opens[0], old_opens[1]),
            "[LB$ {0:2.1f} LS$ {1:2.1f}]".format(self.last_prices[0], self.last_prices[1]))
...
#---------------------------------------------------------
        ...
        prb = self.tick*(low//self.tick) - self.markup*self.tick       
        prs = self.tick*(high//self.tick) + self.markup*self.tick

 
        if high < self.avg_price and (res:=self.pos + old_opens[0]) < 0:
            p = abs(res)
            while (p >0):
                lb_odr =  OrderSamples.LimitOrder("BUY", 1, prb+(abs(self.pos)-p)*self.tick)  
                self.placeOrder(self.nextOrderId(), self.contrakt, lb_odr)
                print("releasing order by {:1.1f}".format(p))
                p -= 1 
            return
        else:
            pass

        if low > self.avg_price and (res:=self.pos - old_opens[1]) > 0:
            p = res
            while (p >0):
                ls_odr =  OrderSamples.LimitOrder("SELL", 1, prs-(abs(self.pos)-p)*self.tick)  
...
#================================================================================================================
    ...
    def contractDetails(self, reqId: int, contractDetails: ContractDetails):
        ...
        if (p:=self.contrakt.multiplier*self.tick) > 0.94:
            print("One tick profit = {:1.2f}".format(p))
            self.markup = 2
        else:
            self.markup = 4
