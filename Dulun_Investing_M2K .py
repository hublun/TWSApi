...
#================================================================
    ...
    def tickDataOperations_req(self):
        ...
        self.reqMktData(1016, ContractSamples.FutureM2K(), "", False, False, [])
        # ! [reqmktdatapreopenbidask]
...
#=====================================================================================================
    ...
    def tickByTickOperations_req(self):
        ...
        self.reqTickByTickData(19004, ContractSamples.FutureM2K(), "BidAsk", 0, False)
        # ! [reqtickbytick]
...
#=====================================================================================
    ...
    def tickByTickBidAsk(self, reqId: int, time: int, bidPrice: float, askPrice: float,
        ...
        #print(#"BidAsk. ReqId:", 
          #      reqId,
        #         "Time:", time, #datetime.datetime.fromtimestamp(time).strftime("%Y%m%d %H:%M:%S"),
        ...
        mean = (askPrice+bidPrice)/2
        self.realTickData_M2K.append(mean)

        size = len(self.realTickData_M2K)
        if size < 5:
            return
        mavg = np.average(self.realTickData_M2K)
        mavg_240 = mavg if size < 241 else np.mean(self.realTickData_M2K[size-240:size-1])
        mavg_1200 = mavg if size < 1201 else np.mean(self.realTickData_M2K[size-1200:size-1])
        std = np.std(self.realTickData_M2K)
        tall = (mean - mavg) / std
        t240 = (mean - mavg_240) / std
        t1200 = (mean - mavg_1200) / std
        tvalue = t1200
        if abs(tvalue) > 3:
            print("| Time: {0:5d} | Pos: {1:2.2f} Avg. Price: {2:4.2f} | Buys: {3:2.0f} Sells: {4:2.0f} | Ask: {5:5.1f} Bid: {6:5.1f} |"
                    .format(time, self.pos_m2k, self.avg_price, self.t_ctr_buy, self.t_ctr_sell,askPrice, bidPrice ))
            print(101*"-")        
            if size>500:
                print(round(tvalue,2) ,size, round(mavg_240, 2), round(mavg_1200,2), 
                    round(mavg, 2), round(std,2))
                if np.sign(tvalue) == 1:
                    self.t_ctr_sell += 1
                else:
                    self.t_ctr_buy += 1
            else:
                print("Priming the trading system !!!")
        else:
            if time%5 == 0:
                print(self.t_ctr_buy, self.t_ctr_sell, 10*"*",time,10*".", round(mean,1)," | ", round(std, 2))
            else:
                pass
    # ! [tickbytickbidask]
...
#==================================================Reat time bar =============================================================
    ...
    def realTimeBarsOperations_req(self):
        ...
        # ! [reqrealtimebars]
        self.reqRealTimeBars(3001, ContractSamples.FutureM2K(), 5, "TRADES", False, [])
        # ! [reqrealtimebars]
   #========================================
    @iswrapper
    # ! [realtimebar]
    def realtimeBar(self, reqId: TickerId, time:int, open_: float, high: float, low: float, close: float,
        ...
        self.cumsum = self.cumsum + close - open_
        if (self.cumsum>1.0):
             self.sells += 1

             print(self.cumsum, self.sells, high,"\n")
             self.cumsum = 0.0
        else:
             if (self.cumsum<-1.0):
                 self.buys += 1

                 print(self.cumsum, self.buys, low,"\n")
                 self.cumsum = 0.0
             else:
                 print(time, self.buys, self.sells, self.t_ctr, self.pos_m2k, self.avg_price)
...
#================================================================================================================
    ...
    def historicalData(self, reqId:int, bar):
        print(reqId, bar)
        self.histData["time"].append(bar.date)
        self.histData['open'].append(bar.open)
        self.histData['high'].append(bar.high)
        self.histData['low'].append(bar.low)
        self.histData['close'].append(bar.close)
        self.histData['avg'].append(bar.average)
        self.histData['vol'].append(bar.volume)
    # ! [historicaldata]
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        #print(self.histData)
        json.dump(self.histData, open("history.json", 'w'))
        #pd.DataFrame.from_dict(self.histData)
    # ! [historicaldataend]
    ...
    def contractOperations(self):
        ...
        self.reqContractDetails(214, ContractSamples.FutureWithLocalSymbol())
